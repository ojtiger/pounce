...
```sh
git clone https://github.com/ojtiger/centre-growl.git
cd centre-growl
make install          # Xcode 필요. 서명은 그 맥의 Apple Development 인증서, 없으면 ad-hoc
open -a CentreGrowl   # 접근성 권한 허용 → 끝
```

또는 GitHub Releases 의 zip 을 받아 /Applications 에 넣고, 처음 한 번은 우클릭 > 열기(공증 전이라 Gatekeeper 가 막음).

## 빌드 / 설치 (개발)

```sh
make install        # 빌드 후 /Applications/CentreGrowl.app 교체
open -a CentreGrowl # 첫 실행 때 접근성 권한을 묻습니다
```
...
2. 공증 자격 저장(한 번만):
   `xcrun notarytool store-credentials centre-growl --apple-id <Apple ID> --team-id <팀 ID> --password <앱 암호>`
3. `make notarize` → `dist/CentreGrowl-<버전>.zip` 과 `.dmg` 가 나옵니다. 유니버설 바이너리, 하드닝 런타임, 공증 스테이플 포함.
