...
⌘+휠로 지난 알림 보기|Show past notifications with ⌘+scroll|⌘+スクロールで過去の通知を表示|用 ⌘+滚轮查看以往通知|用 ⌘+滾輪查看過去的通知|Ver notificaciones anteriores con ⌘+rueda|Revoir les notifications passées avec ⌘+molette|Frühere Mitteilungen mit ⌘+Mausrad anzeigen|Rivedi le notifiche passate con ⌘+rotella|Ver notificações anteriores com ⌘+roda|Показывать прошлые уведомления по ⌘+колесу|Eerdere meldingen bekijken met ⌘+scrollwiel|Pokaż wcześniejsze powiadomienia za pomocą ⌘+kółka|⌘+tekerlek ile geçmiş bildirimleri göster|Xem thông báo cũ bằng ⌘+cuộn|Lihat notifikasi sebelumnya dengan ⌘+gulir
전체 디스크 접근 권한이 필요합니다. 켜면 열리는 설정 창에서 목록 아래 + 를 눌러 응용 프로그램 폴더의 Pounce 를 추가하세요.|Needs Full Disk Access. In the settings window that opens, click + below the list and add Pounce from the Applications folder.|フルディスクアクセスが必要です。開いた設定画面でリストの下の + をクリックし、アプリケーションフォルダの Pounce を追加してください。|需要完全磁盘访问权限。在打开的设置窗口中，点按列表下方的 +，从“应用程序”文件夹添加 Pounce。|需要完整磁碟取用權限。在打開的設定視窗中，按一下列表下方的 +，從「應用程式」檔案夾加入 Pounce。|Requiere acceso total al disco. En la ventana de ajustes que se abre, haz clic en + debajo de la lista y añade Pounce desde la carpeta Aplicaciones.|Nécessite l’accès complet au disque. Dans la fenêtre de réglages qui s’ouvre, cliquez sur + sous la liste et ajoutez Pounce depuis le dossier Applications.|Benötigt Festplattenvollzugriff. Klicke im geöffneten Einstellungsfenster unter der Liste auf + und füge Pounce aus dem Ordner „Programme“ hinzu.|Richiede l’accesso completo al disco. Nella finestra delle impostazioni che si apre, fai clic su + sotto l’elenco e aggiungi Pounce dalla cartella Applicazioni.|Requer acesso total ao disco. Na janela de ajustes que se abre, clique em + abaixo da lista e adicione o Pounce da pasta Aplicativos.|Нужен полный доступ к диску. В открывшемся окне настроек нажмите + под списком и добавьте Pounce из папки «Программы».|Vereist volledige schijftoegang. Klik in het geopende instellingenvenster op + onder de lijst en voeg Pounce toe uit de map Apps.|Wymaga pełnego dostępu do dysku. W otwartym oknie ustawień kliknij + pod listą i dodaj Pounce z folderu Aplikacje.|Tam Disk Erişimi gerekir. Açılan ayarlar penceresinde listenin altındaki + düğmesine tıklayın ve Uygulamalar klasöründen Pounce’u ekleyin.|Cần quyền Truy cập toàn bộ ổ đĩa. Trong cửa sổ cài đặt mở ra, bấm + bên dưới danh sách và thêm Pounce từ thư mục Ứng dụng.|Memerlukan Akses Disk Penuh. Di jendela pengaturan yang terbuka, klik + di bawah daftar lalu tambahkan Pounce dari folder Aplikasi.
...
기본|Default|デフォルト|默认|預設|Predeterminado|Par défaut|Standard|Predefinito|Padrão|Обычный|Standaard|Domyślny|Varsayılan|Mặc định|Bawaan
...
로그 지우기|Clear log|ログを消去|清除日志|清除記錄|Borrar registro|Effacer le journal|Protokoll löschen|Cancella log|Limpar log|Очистить журнал|Log wissen|Wyczyść dziennik|Günlüğü temizle|Xóa nhật ký|Hapus log
